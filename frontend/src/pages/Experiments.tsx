          ...
          return (
            <div className="space-y-3">
              {named.map((exp) => {
                ...
                const meta = list.data?.find((x) => x.id === exp.experiment_id);
                const maxRate = Math.max(
                  ...exp.variants.map((v) => v.recovery_rate),
                  0.01,
                );
                return (
                    ...
                    <CardHeader
                      ...
                      action={
                        <div className="flex items-center gap-2">
                          {meta?.status ? (
                            <Badge
                              tone={meta.status === "active" ? "success" : "neutral"}
                              dot
                            >
                              {meta.status}
                            </Badge>
                        ...
                        {[control, treatment].filter(Boolean).map((v) => (
                          <div key={v!.variant}>
                            ...
                            <p className="mt-1.5 text-2xs text-ink-faint">
                              {num(v!.recovered_events)} of {num(v!.recovery_events)}{" "}
                              recovered · {rupeesCompactFromString(v!.recovered_value)}
                            </p>
                          </div>
                        ))}
                      </div>
                      {exp.absolute_lift != null ? (
